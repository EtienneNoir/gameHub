...
interface FetchGenreHTTResponse {
  count: number;
  results: Genre[];
}
...
const useGenre = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    apiClient
      .get<FetchGenreHTTResponse>("/genres", { signal: controller.signal }) // the signal, controller allows the fetch request to be cancelled if necessary
      .then((res) => {
